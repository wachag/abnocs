...
trait DatabaseLogger extends Logger {
  ...
  override def log(msg: NOCMsg): NOCMsg = {
    ...
    me match {
      case f: Flit =>
        val d = DBIO.seq(Tables.Flits += Tables.FlitsRow(Some(Await.result(f.id,Duration.Inf)),Some(clockCount), Some(f.source), Some(f.dest), Some(f.channel), Some(f.head), Some(f.tail), Some(f.data), Some(sender.path.name)))
